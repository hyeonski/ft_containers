...
template <typename T>
void	printSize(TEST_NS::deque<T> const &vct, bool print_content = 1)
{
	...
	if (print_content)
	{
		typename TEST_NS::deque<T>::const_iterator it = vct.begin();
		typename TEST_NS::deque<T>::const_iterator ite = vct.end();
...
void	checkErase(TEST_NS::deque<TESTED_TYPE> const &vct,
					TEST_NS::deque<TESTED_TYPE>::const_iterator const &it)
{
	static int i = 0;
...
{
	{
		...
		TEST_NS::deque<int>::iterator iter = v.begin();
		iter++;
		iter++;
		...
		for (TEST_NS::deque<int>::iterator iter = v.begin(); iter != v.end(); iter++)
			std::cout << *iter << " ";
		std::cout << std::endl;
		...
		TEST_NS::deque<int> vec_insert1 (3,100);
		TEST_NS::deque<int>::iterator it_insert1;

		for (TEST_NS::deque<int>::iterator iter = vec_insert1.begin(); iter != vec_insert1.end(); iter++)
			std::cout << *iter << " ";
		std::cout << std::endl;
		...
		// std::deque<int> vec_insert2 (2,400);
		...
		TEST_NS::deque<int> vec_assign_first;
		TEST_NS::deque<int> vec_assign_second;
		TEST_NS::deque<int> vec_assign_third;

		vec_assign_first.assign (7,100);             // 7 ints with a value of 100

		TEST_NS::deque<int>::iterator it;
		...
		TEST_NS::deque<int> vec_pop_back;
		int sum (0);
		...
		TEST_NS::deque<int> vec_operator1 (10);   // 10 zero-initialized elements

		TEST_NS::deque<int>::size_type size_operator1 = vec_operator1.size();
		...
		TEST_NS::deque<int> vec_front1;
		//std::deque<int> vec_front2;
		...
		TEST_NS::deque<int> foo (3,100);	 // three ints with a value of 100
		TEST_NS::deque<int> bar (5,200);	 // five ints with a value of 200
		...
		// TEST_NS::deque<int> vec_co_copy(vec_co);

		// for (TEST_NS::deque<int>::iterator iter = vec_co_copy.begin(); iter != vec_co_copy.end(); iter++)
		// 	std::cout << *iter << " ";
		// std::cout << std::endl;

		// TEST_NS::deque<int> vec_co_oper = vec_co;

		// for (TEST_NS::deque<int>::iterator iter = vec_co_oper.begin(); iter != vec_co_oper.end(); iter++)
		// 	std::cout << *iter << " ";
		// std::cout << std::endl;


		// TEST_NS::deque<TESTED_TYPE> vec_copy(5);
		// TEST_NS::deque<TESTED_TYPE>::iterator iter_copy = vec_copy.begin(), iter_copy_end = vec_copy.end();
		...
		// TEST_NS::deque<TESTED_TYPE> vec_copy_range(iter_copy, --(--iter_copy_end));
		// for (int i = 0; iter_copy != iter_copy_end; ++iter_copy)
		// 	*iter_copy = ++i * 5;

		// iter_copy = vec_copy.begin();
		// TEST_NS::deque<TESTED_TYPE> vec_copy_copy(vec_copy);
		// for (int i = 0; iter_copy != iter_copy_end; ++iter_copy)
		// 	*iter_copy = ++i * 7;
		...
		std::cout << "===================== erase test ====================" << std::endl;
		TEST_NS::deque<TESTED_TYPE> vec_erase(10);
		...
		TEST_NS::deque<int> vec_expand;
		vec_expand.push_back(1);

		for (int i=0; i<100; i++)
			vec_expand.push_back(i);

		for (TEST_NS::deque<int>::iterator iter_expand = vec_expand.begin(); iter_expand != vec_expand.end(); iter_expand++)
		...
		std::cout << "===================== push_pop front test ===============" << std::endl;

		TEST_NS::deque<int> push_pop;
		push_pop.push_front(1);

		for (int i=0; i<100; i++)
			push_pop.push_front(i);

		for (TEST_NS::deque<int>::iterator iter_push_pop = push_pop.begin(); iter_push_pop != push_pop.end(); iter_push_pop++)
			std::cout << *iter_push_pop << " ";
		std::cout << std::endl;
		...
		std::cout << "===================== push_pop front test end ===========" << std::endl;
	}
	...
	if (LEAK_TEST != 0)
	{
		system("leaks a.out | grep leaked 1>&2 ");
		system("leaks a.out 1>&2 ");
	}
	return 0;
}